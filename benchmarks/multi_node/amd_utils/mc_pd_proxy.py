    ...
    try:
        ...
        try:
            ...
            if r.status_code >= 400:
                print(f"[proxy] PREFILL {endpoint} -> {r.status_code}: {r.text[:600]}", flush=True)
            r.raise_for_status()
        ...
        if is_stream:
            # Probe non-stream first so we can surface a clean upstream error
            # (status+body) instead of a half-open 200 stream that hides the cause.
            async def gen():
                async with dc.stream("POST", endpoint, json=req, headers=_headers()) as resp:
                    if resp.status_code >= 400:
                        body = (await resp.aread())[:600]
                        print(f"[proxy] DECODE(stream) {endpoint} -> {resp.status_code}: {body!r}", flush=True)
            ...
            if r.status_code >= 400:
                print(f"[proxy] DECODE {endpoint} -> {r.status_code}: {r.text[:800]}", flush=True)
                # Surface the upstream status + body so aiperf's error summary is actionable.
                return JSONResponse(content={"error": "decoder_error", "upstream_status": r.status_code, "upstream_body": r.text[:2000]}, status_code=r.status_code)
